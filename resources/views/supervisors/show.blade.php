<x-app-layout>
    ...
    <div class="bg-white rounded-4 shadow-sm p-4 mb-5">
        <div class="d-grid gap-3 d-sm-flex flex-sm-row-reverse">
            <div class="d-grid gap-3 d-sm-flex">
                ...
                <button type="button" class="btn btn-danger btn-action-icon" data-bs-toggle="modal" data-bs-target="#modalDelete{{ $supervisor->id }}">
    ...
    <div class="bg-white rounded-4 shadow-sm p-4 mb-4">
        <div class="d-flex flex-column flex-xl-row align-items-start">
            ...
            <div class="col-md-4 text-center mb-4 mb-xl-0">
                @if ($supervisor->image)
                    <img src="{{ asset('storage/public/supervisors/' . $supervisor->image) }}"
            ...
            <div class="flex-grow-1 ms-xl-5 w-100">
                <div class="table-responsive">
                    <table class="table table-striped lh-lg">
                        <tr><td width="220">Kategori</td><td width="10">:</td><td>{{ $supervisor->category->name ?? '-' }}</td></tr>
                        <tr><td>Nama Lengkap</td><td>:</td><td>{{ $supervisor->nama }}</td></tr>
                        <tr><td>Jenis Kelamin</td><td>:</td><td>{{ $supervisor->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td></tr>
                        <tr><td>Umur</td><td>:</td><td>{{ $supervisor->umur ?? '-' }}</td></tr>
                        <tr><td>Mata Pencaharian</td><td>:</td><td>{{ $supervisor->mata_pencaharian ?? '-' }}</td></tr>
                        <tr><td>Tempat Tinggal</td><td>:</td><td>{{ $supervisor->tempat_tinggal ?? '-' }}</td></tr>
                        <tr><td>No. Anggota Koperasi</td><td>:</td><td>{{ $supervisor->no_anggota_koperasi ?? '-' }}</td></tr>
                        <tr><td>Jabatan</td><td>:</td><td>{{ $supervisor->jabatan ?? '-' }}</td></tr>
                        <tr><td>Tanggal Dipilih</td><td>:</td><td>{{ $supervisor->tanggal_dipilih ? date('d-m-Y', strtotime($supervisor->tanggal_dipilih)) : '-' }}</td></tr>
                        <tr><td>Tanggal Berhenti</td><td>:</td><td>{{ $supervisor->tanggal_berhenti ? date('d-m-Y', strtotime($supervisor->tanggal_berhenti)) : '-' }}</td></tr>
                        <tr><td>Keterangan</td><td>:</td><td>{{ $supervisor->keterangan ?? '-' }}</td></tr>
                    </table>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalDelete{{ $supervisor->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">

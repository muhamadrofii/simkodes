<x-app-layout>
        ...
        <div class="d-grid gap-3 d-sm-flex flex-sm-row-reverse">
            <div class="d-grid gap-3 d-sm-flex">
                ...
                <button type="button" class="btn btn-danger btn-action-icon" data-bs-toggle="modal"
                    data-bs-target="#modalDelete{{ $officer->id }}">
    ...
    <div class="bg-white rounded-4 shadow-sm p-4 mb-4">
        <div class="d-flex flex-column flex-xl-row align-items-start">
            ...
            <div class="col-md-4 text-center mb-4 mb-xl-0">
                @if ($officer->image)
                    <img src="{{ asset('officer_files/' . $officer->image) }}" class="img-thumbnail rounded-5 shadow-sm"
                        width="150" alt="{{ $officer->nama }}">
                @else
                ...
                @if ($officer->ttd)
                    <div class="mt-3">
                        <img src="{{ asset('officer_files/' . $officer->ttd) }}" class="img-thumbnail rounded-4 shadow-sm"
                            width="120" alt="TTD">
                ...
                <div class="table-responsive">
                    <table class="table table-striped lh-lg">
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td>{{ $officer->no_anggota_koperasi ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td>Tempat Tinggal</td>
                            <td>:</td>
                            <td>{{ $officer->tempat_tinggal ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td>Tanggal Diangkat</td>
                            <td>:</td>
                            <td>{{ $officer->tanggal_diangkat ? date('d-m-Y', strtotime($officer->tanggal_diangkat)) : '-' }}
                            </td>
                        </tr>
                        <tr>
                            <td>Tanggal Berhenti</td>
                            <td>:</td>
                            <td>{{ $officer->tanggal_berhenti ? date('d-m-Y', strtotime($officer->tanggal_berhenti)) : '-' }}
                            </td>
                        </tr>
                        <tr>
                            <td>Keterangan</td>
                            <td>:</td>
                            <td>{{ $officer->keterangan ?? '-' }}</td>
                        </tr>
                    </table>
                </div>
            </div>
    ...
    <div class="modal fade" id="modalDelete{{ $officer->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
    ...
    {{-- Tombol Cetak KTA --}}
    <a href="{{ route('officers.kta', $officer->id) }}" class="btn btn-success btn-action-icon mt-3" target="_blank">
